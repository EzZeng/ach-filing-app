...
type Tab = string; // format code | "refs" | "schema" | "help"
...
function HomePage() {
  ...
  useEffect(() => {
    ...
    setTab((t) => (t && (t === "refs" || t === "schema" || t === "help" || formats[t]) ? t : initial));
  ...
  return (
    <div className="app-shell">
      ...
      <footer className="border-t border-border bg-surface/80 px-4 py-3 text-center text-xs text-muted">
        格式由 JSON 參數驅動 · 本工具僅供建檔輔助，上傳前請自行核對
      </footer>
    </div>
  );
}
